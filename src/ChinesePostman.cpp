 ...
 *
**/
ChinesePostman::ChinesePostman(RegionData& data, ReebGraph& graph, std::list<Edge>& eulerCycle, vector<Point2D>& wayPoints)
{
  lp = NULL;
 ...
 *   None 
**/
void ChinesePostman::addPairToEndPairings( \
 ...
 */
void ChinesePostman::solveCPP(RegionData& data, ReebGraph& graph, std::list<Edge>& eulerCycle) 
{
  cv::Mat map = data.map;
  eulerCycle.clear();
...
*/
      ...
      double sr,er;
      if(graph.getVProp(_v1).x>graph.getVProp(_v2).x) swap(_v1,_v2);
      ...
      _t = graph.getEProp(*ei).topBoundary[graph.getEProp(*ei).topBoundary.size()-1].ycoord();
      _b = graph.getEProp(*ei).bottomBoundary[graph.getEProp(*ei).topBoundary.size()-1].ycoord();
      ...
      for(unsigned int j = 0;j<graph.getEProp(*ei).topBoundary.size();++j,sr += step)
      {
  ...
  {
    /* Step 1 :
     * Let n' be the node other than n incident to edge e. If there is an edge e'
     * with an end meeting n' which is not yet paired, go to Step 2. Otherwise,
     * n' must be equal to n0. In that case, form the edge pairs (el,e0) and
     * (e,e2) meeting node n0. Go to Step 3. */
    ...
    for( list<Edge>::iterator eit = unpairedEdges.begin(); eit != unpairedEdges.end(); ++eit )
    {
      ...
      if( (_v1==n_ || _v2==n_) && e!=*eit )
      {
        ...
        Point2D _me0 = (_prob_e.topBoundary.front()+_prob_e.bottomBoundary.front())/2;
        Point2D _me1 = (_prob_e.topBoundary.back()+_prob_e.bottomBoundary.back())/2;
        Point2D _met0 = (_prob_eit.topBoundary.front()+_prob_eit.bottomBoundary.front())/2;
        Point2D _met1 = (_prob_eit.topBoundary.back()+_prob_eit.bottomBoundary.back())/2;
    ...
    {
      /* Step 2 :
       * Pair the edges e and e' meeting n'. Change n to be n' and e to be e'. Go to Step 1. */
      n = n_;
      addPairToEndPairings( e, e_, n , &unpairedEdges, &pairedEdges, &EndPairs);
      e = e_;
      /* Goto Step 1 */
    }
    ...
    else
    {
      ...
      addPairToEndPairings( e1, e0, n , &unpairedEdges, &pairedEdges, &EndPairs );
      addPairToEndPairings( e, e2, n , &unpairedEdges, &pairedEdges, &EndPairs );

      /* Step 3 :
       * Change n0 to be any node which has at least one pair (e1,e2) of edges
       * meeting it and at least one unpaired edge e0 also meeting it. Let n
       * be set equal to n0 and e be set equal to e0, and go to Step 1. If no
       * such node n0 exists, terminate.
       */

      bool found_new_n0 = false;  /* And add two previous last points for continuity reasons.*/
      list< pair<pair<Edge,Edge>,Vertex> >::iterator bestPair;
      double min_dist = (map.cols + map.rows)*(map.cols + map.rows);
      for( list< pair<pair<Edge,Edge>,Vertex> >::iterator pit=EndPairs.begin(); \
          pit!=EndPairs.end(); ++pit )
      {
      ...
      if( found_new_n0 )
      {
        /* Remove the current pair to attach a new cycle, not necessary but
  ...
  do
  {
    for( list< pair<pair<Edge,Edge>,Vertex> >::reverse_iterator pit=EndPairs.rbegin(); \
        pit!=EndPairs.rend(); ++pit )
 ...
 *
**/
void ChinesePostman::viewEulerGraph(QString fileName, RegionData& data, ReebGraph& graph, std::list<Edge>& eulerCycle, vector<Point2D>& wayPoints) 
{
  if(graph.empty() == true)
  {
